...
<html xmlns="http://www.w3.org/1999/xhtml"><!-- InstanceBegin template="/Templates/properties.dwt.php" codeOutsideHTMLIsLocked="false" -->
<head>
...
.background .container .content center #propertypanel table tr td form input {
...
<div class="container">
  ...
  <div class="content">
    ...
    <?
		...
		$last_modified = getLastEdit();
		//print it all out
		print("<p><font size=\"-1\">Last Modififed: ");
		print($last_modified);
		print("</p></font>");
   ?>
    ...
    <?
	$fetch = mysql_query("SELECT * FROM properties");
	$cnt = mysql_num_rows($fetch);
	$fetch = mysql_query("SELECT * FROM properties WHERE listed = '1'");
	$availcnt = mysql_num_rows($fetch);
	if($availcnt == 0)
	{
		?><p>There are no available properties at this time.<?
	}
	if($cnt > 0){
		?>
<center>
          <?
		$result = mysql_query("SELECT * FROM properties WHERE listed='1'");
        while ($row = mysql_fetch_assoc($result)) {
			...
			$rent = $row['rent'];
			$sdesc = $row['shortdescription'];
			$img1 = $row['img1'];
			$img2 = $row['img2'];
			$img3 = $row['img3'];
			$url = NULL;
			if($sdesc == NULL){
				$sdesc = "No Description Entered";
			}if($img1 == NULL){
            ...
            <div id="propertypanel">
  			...
  			<table width="724" Height="215" border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td rowspan="3"><img src="<?php echo $url; ?>" width="214" height="136" alt="<?php echo $sdesc; ?>"></td>
              <td colspan="2"><p><span class ="linktext2"><a href="http://www.setag.net/viewpage.php?propid=<? echo $id; ?>"><? echo $sdesc; ?></a></span></p></td>
              
            </tr>
            <tr>
              <td width="247"><span id="font4"><p>in <? echo $location; ?></p></span></td>
              <td width="51"><span id="font5"><p><? echo "(".$id.")"; ?><p></span></td>
            </tr>
            <tr>
              <td height="102"><span id="font4"><p>Rent :<strong><? echo " $".$rent; ?></strong><br />
              Beds: <? echo $rooms; ?> , Bathroom(s): <? echo $bath; ?></p></span></td>
              <td colspan="2"></td>
              <td width="197"><form action="viewpage.php" method="get" target="_new"> <input name="propid" type="hidden" value="<? echo $id; ?>" /><input name="submit" type="image" id="viewbutton" src="images/viewbutton.png"/></form>(636)220-7501</td>
            </tr>
            <tr>
              <td colspan="5"><center></center>                <center>
                </center>                <center>
              </center>                <? echoLD($id); ?></td>
              </tr>
            </table>
